...
class CalendarSettingsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        if not self.holidays and default_holidays:
            inherit_row = QHBoxLayout()
            inherit_row.addWidget(QLabel(
                f"This project has no holidays, but global defaults have {len(default_holidays)}."))
            copy_btn = QPushButton(f"Copy global holidays ({len(default_holidays)})")
            copy_btn.clicked.connect(lambda: self.copy_global_holidays(default_holidays))
            inherit_row.addWidget(copy_btn)
            layout.addLayout(inherit_row)
    ...
    def copy_global_holidays(self, default_holidays):
